...
# Load Companies and rounds2 into datasets

companies <- read.delim("companies.txt", header = TRUE,stringsAsFactors = F)
rounds2 <- read.csv("rounds2.csv", header = TRUE,sep=",")

#Lowering the cases of both the data sets to merge them with commom primary key

companies$permalink <- tolower(companies$permalink)
rounds2$company_permalink <- tolower(rounds2$company_permalink)

#Unique companies present in rounds2 dataset
count(distinct(rounds2,company_permalink))

#Unique companies present in companies dataset
...
#Removing all the NA values from the data frame, as they are not going to contribute
#any useful results to the analysis. NAs in raised_amount_usd needs to be ignored and also other columns
#as well.
...
# Obtaining the Sector wise investment count and total investment for each of the
#three countries
